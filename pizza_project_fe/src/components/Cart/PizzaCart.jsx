...
const PizzaCart = (props) => {
  ...
  const handleDeleteItem = () => {
    dispatch(deleteCart({hiddenDelete: false, type: 'pizza', index: index, itemName: pizza.pizza.pizzaType.name}))
  }
  const handleChangeQuantity = () => {
    dispatch(updateQuantity({hiddenChange: false, type: 'pizza', index: index, itemName: pizza.pizza.pizzaType.name, quantity: pizza.quantity}))
  }
  return (
    <div key={index} className='relative py-2 px-10 cursor-pointer hover:bg-slate-100 delay-100 text-sm'>
        <div className='flex justify-between font-bold pb-1'>
          <div>{pizza.pizza.pizzaType.name} </div>
